...
function ParticleCanvas({ flip }: { flip?: boolean }) {
  ...
  return (
    <canvas
      ...
      width={60}
      height={80}
      style={{ opacity: 0.85, transform: flip ? "rotate(90deg) scaleX(-1)" : "rotate(-90deg)" } as any}
    />
  );
}
...
export function ExploreParticles() {
  ...
  return (
    <View style={{ position: "absolute", top: 0, left: -62, right: -62, bottom: 0, flexDirection: "row", justifyContent: "space-between", pointerEvents: "none" } as any}>
      <ParticleCanvas />
      <ParticleCanvas flip />
    </View>
  );
